...
echo "button =  speeddial, \"Random Call\", 726366"  >> /etc/asterisk/sccp-phones.conf
...
echo "SCCP/$PHONENUM" >> $EXTENIONS_LIST_FILE

# Plaintext Directory for Phone List
echo "$LISTEDNAME $CALLERIDNAME $PHONENUM ${DIRECTORY_FILE[$DIRECTORY]}" >> $PLAINTEXT_DIRECTORY_FILE
fi 


# Put Info In Button List
echo "button = speeddial, $LISTEDNAME, $PHONENUM, $PHONENUM@hints" >> $BUTTONS_LIST_FILE
